...
def affine_relu_do_forward(x, w, b, do_params):
    ...
    out, do_cache = dropout_forward(out_relu, do_params )
    cache = (fc_cache, relu_cache, do_params)
    return out, cache
...
def affine_relu_do_backward(dout, cache):
    ...
    Backward pass for the affine-relu convenience layer
    """
    fc_cache, relu_cache, do_params = cache
    dout_do = dropout_backward(dout, do_params)
    da = relu_backward(dout_do, relu_cache)
    dx, dw, db = affine_backward(da, fc_cache)
    return dx, dw, db
